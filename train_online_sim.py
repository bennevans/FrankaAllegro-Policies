...
class Workspace:
    ...
    def _get_act_obs(self, time_step):
        ...
        if 'tactile' in self.data_reprs:
            obs_dict['tactile_repr'] = torch.FloatTensor(time_step.observation['tactile'])
        if 'allegro' in self.data_reprs or 'kinova' in self.data_reprs or 'franka' in self.data_reprs:
    ...
    def train_online(self):
        ...
        while train_until_step(self.global_step): 
            ...
            if time_step.last() or is_episode_done:
                ...
                x = float(input("Enter the reward"))
                # Get the rewards
                ...
                self.train_video_recorder.save(f'{ts}_e{self.global_episode}_f{self.global_frame}_r{round(new_rewards,2)}.mp4')
                
                # Update the reward in the timesteps accordingly
                obs_length = len(time_steps)
                for i, elt in enumerate(time_steps):
                        if i ==len(time_steps)-1:
                            old_reward = elt.reward
                            new_reward = 10*(old_reward + new_rewards)
                            elt = elt._replace(reward=new_reward) # Up
