...
if __name__ == '__main__': 
	...
	policy = policies.SawyerDrawerOpenV2Policy()
	video_recorder = VideoRecorder(Path('.'))

	time_step = env.reset()
	print(f'time_step: {time_step}')
	print(time_step.observation['pixels'].shape, time_step.observation['features'].shape)
	for i in range(125): 
		observation = time_step.observation
		action = policy.get_action(observation['features'])
