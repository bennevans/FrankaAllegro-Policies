...
class TactileCurvedRepresentation(TactileRepresentation):
    def __init__(
        self,
        encoder_out_dim,
        tactile_encoder,
        ...
        assert representation_type == 'tdex', 'For curved finger tactile readings representation types other than T-Dex is not implemented'
        super().__init__(encoder_out_dim=encoder_out_dim, 
    ...
    def _get_tdex_repr(self, tactile_values, detach=True):
        img = self.tactile_image.get(type=None, tactile_values=tactile_values)
        img = self.transform(img).to(self.device)
        if detach:
            return self.encoder(img.unsqueeze(0)).squeeze().detach().cpu().numpy()
        return self.encoder(img.unsqueeze(0)).squeeze()