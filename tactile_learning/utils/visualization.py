...
def dump_curved_tactile_state(tactile_values):
    plotter = XelaCurvedPlotter()
    plotter.draw(
        palm_sensor_values=tactile_values['palm_values'],
        fingertip_sensor_values=tactile_values['fingertip_values'],
        finger_sensor_values=tactile_values['finger_values'],
        figure_plot_path='tactile_state.png'
    )
...
def dump_whole_state(vision_state, hand_tip=None, arm_cart=None, tactile_values=None, tactile_image=None,  title='curr_state'):
    cv2.imwrite(f'{title}_vision.png', vision_state)
    vision_img = cv2.imread(f'{title}_vision.png')

    if not tactile_values is None and not tactile_image is None:
        dump_curved_tactile_state(tactile_values)
    ...
    elif not arm_cart is None and not hand_tip is None:
        dump_robot_state(hand_tip, arm_cart)
        robot_state = cv2.imread('robot_state.png')
        state_img = concat_imgs(vision_img, robot_state, orientation='horizontal')
