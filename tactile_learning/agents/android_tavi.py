...
class AndroidTAVI(AndroidAgent):
    def __init__(self,
        ...
        robot_data_path, robot_demo_nums, 
        ...
        **kwargs
        ...
        super().__init__(
            ...
            robot_demo_nums = robot_demo_nums,
    ...
    def base_act(self, obs, episode_step): # Returns the action for the base policy - openloop
        ...
        human_action, is_done = self.base_policy.act( # TODO: Make sure these are good
            obs, episode_step
        )
        
        ## TODO: change the output to be robot rewarder action + human base action
        ## NOTE: this need to be changed in the base policy
        return torch.FloatTensor(human_action).to(self.device).unsqueeze(0), is_done


    def act(self, obs, global_step, episode_step, eval_mode, metrics=None):
        with torch.no_grad():
            human_base_action, is_done = self.base_act(obs, episode_step)

        print('base_action.shape: {}'.format(human_base_action.shape))
        ...
        dist = self.actor(obs, human_base_action, stddev)
        if eval_mode:
        ...
        action = human_base_action + offset_action
        ...
        return action.cpu().numpy()[0], human_base_action.cpu().numpy()[0], is_done, metrics
