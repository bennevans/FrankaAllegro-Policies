...
class Workspace:
    ...
    def save_snapshot_every_ten_episodes(self):
        snapshot = self.work_dir / 'checkpoints' 
        snapshot.mkdir(parents=True, exist_ok=True)
        snapshot = snapshot / ('snapshot_{}.pt'.format(self.global_episode))
        keys_to_save = ['_global_step', '_global_episode']
        payload = {k: self.__dict__[k] for k in keys_to_save}
        payload.update(self.agent.save_snapshot())
        with snapshot.open('wb') as f:
        ...
        while train_until_step(self.global_step): 
            ...
            if time_step.last() or is_episode_done:
                ...
                if self.global_episode % 10 == 0:
                    self.save_snapshot_every_ten_episodes()
