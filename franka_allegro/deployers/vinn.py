...
class VINN(Deployer):
    def __init__(
        ...
        if not deployment_dump_dir is None: # Either way should create this directory if it's given
    ...
    def _get_knn_action(self, state_dict, visualize=False):
        ...
        state_dict['image'] = self._get_curr_image()
        ...
        for data in self.data_reprs:
            ...
            if data == 'image':
                repr_data[data] = state_dict['image']
        ...
        if self.dump_deployment_info:
            ...
            self.deployment_info['tactile_values'].append(state_dict['tactile'])
        ...
        if visualize: 
            ...
            self.visualization_info['tactile_values'].append(state_dict['tactile'])
    ...
    def _visualize_state(self, curr_tactile_values, curr_fingertip_position, curr_kinova_cart_pos, id_of_nn, nn_idxs, nn_separate_dists, state_id=None, image=None):
        ...
        dump_whole_state(vision_state=curr_image_cv2,
                         tactile_values=curr_tactile_values,
                         tactile_image=curr_tactile_image,
                         hand_tip=None, # NOTE: If not used delete these soon
                         arm_cart=None,
                         title='curr_state')
        dump_whole_state(vision_state=knn_vis_data['image'],
                         tactile_values=knn_vis_data['tactile_values'],
                         tactile_image=knn_vis_data['tactile_image'],
                         hand_tip=None,
                         arm_cart=None,
                         title='knn_state', )
    ...
    def _get_data_with_id(self, id, visualize=False):
        ...
        for data in self.data_reprs:
            ...
            if data == 'tactile':
                ...
                if isinstance(self.data[data]['values'][demo_id], dict): # Means that this the curved finger
                    tactile_value = dict(
                ...
                else: # Means it is the flat finger
